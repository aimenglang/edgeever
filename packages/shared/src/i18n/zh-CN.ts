...
export const zhCN = {
  ...
  login: {
    title: "登录 我的笔记",
    subtitle: "我的笔记 - 快乐笔记，边缘存储",
    username: "账号",
    password: "密码",
    submitting: "安全登录中...",
    desktopStarting: "正在启动 我的笔记",
    ...
    instanceApiNotFound: "这个地址不是 我的笔记 实例，请检查实例地址。",
    ...
    securityPolicyBlocked: "登录请求在到达 我的笔记 前被安全策略拦截。请实例管理员检查 Cloudflare Security Events、Access、WAF 或反向代理规则。",
    ...
    invalidInstanceResponse: "这个地址不是有效的 我的笔记 实例，请检查实例地址。",
    ...
    desktopInstanceTitle: "连接 我的笔记 实例",
  ...
  notebookPane: {
    ...
    syncDetails: {
      ...
      recover: "恢复为新笔记",
      recoverTitle: "恢复本地修改",
      recoverDescription: "选择目标笔记本。我的笔记 会保留失败项中的内容并创建一条新笔记，然后继续同步。",
      ...
      reportDiagnosticsNotice: "以下诊断由 我的笔记 在本地生成并脱敏，请在提交前检查。",
      operations: {
        memo_create: "新建笔记",
        memo_update: "更新笔记",
        memo_delete: "删除笔记",
  ...
  settings: {
    ...
    customEditorTheme: {
      ...
      importErrors: {
        invalidFile: "无法导入：文件不是有效的 我的笔记 编辑器主题。",
  ...
  plugins: {
    ...
    updates: {
      ...
      confirmDescription: "我的笔记 不会自动安装更新。确认后将下载并替换当前版本，原启用状态保持不变。",
  ...
  objectStorage: {
    title: "OSS 对象存储",
    description: "配置整个 我的笔记 实例用于新图片和附件的对象存储。仅实例 Owner 可以修改。",
    providers: {
  ...
  aiModel: {
    ...
    apiKey: "API Key",
    apiKeyCreateHint: "凭据将由 我的笔记 服务端加密保存。",
  ...
  systemInfo: {
    ...
    clientSection: "当前客户端",
    clientSectionDescription: "这台设备上的 我的笔记 应用与运行环境。",
  ...
  dataExport: {
    ...
    webdavUsername: "用户名",
    webdavPassword: "密码",
    webdavSecurityNote: "启用自动备份后，WebDAV 凭据会保存在当前浏览器中；密码不会发送到 我的笔记 服务器。自动备份会在 我的笔记 页面或 PWA 运行期间检查，应用完全关闭时浏览器无法保证执行。WebDAV 服务需要允许当前域名的 CORS 请求。",
    webdavManualSecurityNote: "地址和用户名会保存在当前浏览器中；密码仅用于当前手动操作，不会发送到 我的笔记 服务器。WebDAV 服务需要允许当前域名的 CORS 请求。",
    ...
    importComplete: "我的笔记 ZIP 导入完成。",
    importErrors: {
      invalidZip: "无法解压这个文件：它不是有效的 ZIP，或文件已经损坏。",
      missingManifest: "无法识别这个文件：压缩包中缺少 manifest.json。",
      unsupportedFormat: "不支持这个压缩包格式：请选择由 我的笔记 导出的 ZIP。",
      unsupportedVersion: "当前版本无法导入这个 我的笔记 ZIP，请先升级 EdgeEver。",
      ...
      network: "导入时无法连接 我的笔记 服务，请检查网络连接后重试。",
      unknown: "导入失败，发生了无法识别的错误。请重新导出后再试。",
    },
    importConfirmTitle: "导入 我的笔记 ZIP",
    importConfirmDescription: "文件包含 {{memos}} 篇笔记和 {{resources}} 个资源。导入会保留当前实例中的其他数据，但会覆盖相同 我的笔记 ID 的内容。",
    confirmImport: "确认导入",
  },
  shortcuts: {
    title: "绑定快捷键",
  ...
  editor: {
    ...
    saveState: {
      ...
      error: "保存失败",
      unsaved: "未保存",
      storageUnavailable: "浏览器本地存储无响应。请保持当前页面打开，关闭其他 我的笔记 标签页后重试。清除浏览器数据前请先复制本地草稿。",
      storageRetry: "重试保存",
      conflictReason: {
    ...
    pdfExport: {
      ...
      close: "关闭",
      error: "打印预览生成失败",
      popupBlocked: "浏览器阻止了打印预览窗口，请允许 我的笔记 打开弹出式窗口后重试。",
    },
    save: "保存",
    more: "更多",
    moreAria: "笔记更多操作",
  ...
  localDataReset: {
    title: "危险操作",
    description: "清除这台 Mac 上的所有 我的笔记 本地数据，并恢复到首次使用状态。",
    action: "清除本地数据",
    dialogTitle: "清除这台 Mac 上的 我的笔记 数据？",
    ...
    errors: {
      ...
      applicationBundleNotFound: "无法识别清除后需要重新启动的已安装应用。请将 我的笔记 重新安装到“应用程序”后再试。",
  ...
  advancedPlay: {
    ...
    prompts: {
      persona: {
        title: "人物画像",
        prompt: "请通过 我的笔记 MCP 读取我的笔记，基于真实笔记内容为我整理一份人物画像。请只根据笔记中的证据判断，不要做心理诊断，不要夸张定性。输出包括：长期关注的主题、做事偏好、能力线索、反复出现的问题、近期动向，并在每条结论后列出相关笔记标题或 memo id。",
      },
      knowledgeMap: {
        title: "知识图谱",
        prompt: "请通过 我的笔记 MCP 读取我的笔记，为我整理一份知识地图。请找出主要知识领域、每个领域下的关键概念、相关笔记、我已经掌握的部分和还需要补齐的问题。输出结构要适合后续继续学习和写作。",
      },
      tagAdvice: {
        title: "标签建议",
        prompt: "请通过 我的笔记 MCP 读取我的笔记和现有标签，帮我设计一套更清晰的标签体系。请指出重复、过细、过宽或命名不一致的标签，并给出合并、重命名和新增标签建议。先不要修改笔记，等我确认后再执行。",
      },
    },
  },
  templates: {
  ...
  pwa: {
    ...
    sidebarDownloadsTitle: "下载 我的笔记 客户端与浏览器插件",
    ...
    sidebarAndroidTitle: "在 Google Play 下载 我的笔记 安卓端",
    sidebarIos: "iOS",
    sidebarIosBadge: "App Store",
    sidebarIosRegionBadge: "非大陆区",
    sidebarIosTitle: "在 App Store 下载 我的笔记 iOS 端（仅支持非大陆区 Apple ID）",
    ...
    sidebarClipperTitle: "安装 我的笔记 浏览器剪藏插件",
    iosPrompt: {
      title: "将 我的笔记 安装到主屏幕",
